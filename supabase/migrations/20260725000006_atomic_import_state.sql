-- P4 fix: make the one-time localStorage → Postgres import atomic.
--
-- `importState` was the last multi-table writer that could half-apply. It issued
-- up to EIGHT sequential REST inserts — friends, groups, group_members, expenses,
-- expense_payers, expense_splits, settlements, activity_events — and each REST
-- call is its own Postgres transaction. There was no try/catch, no compensating
-- delete and no batch tag, so the import had seven distinct half-applied stopping
-- points, and afterwards nothing in the database distinguished a row that landed
-- from one the user had entered by hand. The user's words for the bug: "I don't
-- want any midway failures to leave me hunting for expenses."
--
-- The compensating-cleanup approach used by the CSV importer (`importCsvBatch`
-- tags every row with a batch id and deletes by that tag on failure) cannot cover
-- this path: as of 20260724000001 `activity_events` has only SELECT and INSERT
-- policies — it is append-only by design — so a client-side DELETE of the events
-- it just wrote is structurally impossible. Step 8 is uncleanable from the client.
-- A single transaction is the only construction that covers all eight steps.
--
-- Transactions are not reachable over the REST API, but a plpgsql body IS one:
-- every statement below runs in one transaction, so the import either lands whole
-- or leaves the account exactly as it was. That also makes a retry after a failure
-- safe, which matters because `remapLocalState` mints fresh uuids on every attempt
-- — under the old code, retrying a partially-applied import inserted a second copy
-- of everything that had already landed.
--
-- ---------- why `security invoker` (and never definer) ----------
--
-- Identical reasoning to `update_expense_with_children` (20260725000002) and
-- `update_group_with_members` (20260725000003), and it matters MORE here: every
-- single id in this payload — the row ids, the parent refs, and `owner_id` itself
-- — is chosen by the caller, and there are eight tables in reach. `security
-- definer` would run the body as the function owner and bypass RLS, turning this
-- into an unrestricted cross-tenant write primitive: anyone could stuff rows into
-- anyone's account, including forged entries in the append-only audit log.
--
-- `security invoker` (the default, spelled out because it is load-bearing) keeps
-- `auth.uid()` equal to the calling user, so every statement below is filtered by
-- the same policies that gate direct REST writes: `friends_own`, `groups_own`,
-- `expenses_own`, `settlements_own` and `activity_events_insert_own` compare
-- `owner_id` to `auth.uid()`, and `group_members_via_parent` /
-- `expense_payers_via_parent` / `expense_splits_via_parent` gate the child tables
-- on the parent row's owner. This function therefore grants no authority the
-- caller did not already have; it only makes their own write atomic. Atomicity
-- needs a single transaction, not elevated privileges. Do not "fix" a permission
-- error here by switching to definer — a permission error means RLS is working.
--
-- ---------- forward compatibility ----------
--
-- Every INSERT names its columns explicitly rather than relying on column order
-- or on `jsonb_populate_record` over the whole row type, so a column added to one
-- of these tables by a later migration neither breaks this function nor silently
-- receives a NULL that overrides its default. The cost is that a new column is not
-- imported until it is named here — see the note on `expenses` below.

create function public.import_state(
...
begin
  ...
  -- ---------- 4. expenses ----------
  -- Columns are named explicitly. NOTE for whoever lands the `split_mode` column:
  -- add it to this list (and to the client payload) or imported expenses will
  -- silently take the column default instead of the split mode the user chose.
  with ins as (
